...
public class MachineManager implements IMachineManager {
     ...
     * Placement manager
     */
    private CloudProvider selectCloudProvider(final MachineTemplate mt)
        ...
        if (cp.size() == 0) {
            throw new CloudProviderException(
                    " No matching cloud provider for template " + mt.getId());
        }
        return cp.get(0);
    }

    private CloudProviderAccount getCloudProviderAccount(
    ...
    private void addVolumes(final Job parent, final Machine m,
            final List<MachineVolumeTemplate> vts) {
    ...
    private void attachVolumes(final Job j, final Machine m,
            final List<MachineVolume> volumes) {
    ...
    public Job createMachine(final MachineCreate machineCreate)
        ...
        try {
            UtilsForManagers.emitJobListenerMessage(
                    jobCreateMachine.getProviderAssignedId(), this.ctx);
        } catch (Exception e) {
    ...
    public List<Machine> getMachines() throws CloudProviderException {
        return UtilsForManagers.getEntityList("Machine", this.em, this
                .getUser().getUsername());
    }
    ...
    private Job doService(final String machineId, final String action)
        ...
        if (j.getStatus() == Job.Status.RUNNING) {
            try {
                UtilsForManagers.emitJobListenerMessage(
                        job.getProviderAssignedId(), this.ctx);
            } catch (Exception e) {
                throw new ServiceUnavailableException(e.getMessage() + "  "
                        + action);
            }
    ...
    public List<MachineConfiguration> getMachineConfigurations()
        ...
        return this.em
                .createQuery(
                        "SELECT c FROM MachineConfiguration c WHERE c.user.id=:userid")
    ...
    public List<MachineConfiguration> getMachineConfigurations(final int first,
        ...
        Query query = this.em
                .createQuery("FROM MachineConfiguration v WHERE v.user.username=:username ORDER BY v.id");
        query.setParameter("username", user.getUsername());
        query.setMaxResults(last - first + 1);
        query.setFirstResult(first);
        return query.setFirstResult(first).setMaxResults(last - first + 1)
                .getResultList();
    }
    ...
    public List<MachineConfiguration> getMachineConfigurations(
        ...
        return this.em
                .createQuery(
                        "FROM MachineConfiguration v WHERE v.user.username=:username ORDER BY v.id")
    ...
    public MachineTemplate createMachineTemplate(final MachineTemplate mt)
        ...
        if (mi == null || mi.getState() == MachineImage.State.DELETED) {
            throw new InvalidRequestException(
                    "Invalid reference to machine image " + mi.getId());
        }
    ...
    public List<MachineTemplate> getMachineTemplates()
        ...
        return this.em
                .createQuery(
                        "SELECT c FROM MachineTemplate c WHERE c.user.id=:userid")
                .setParameter("userid", this.getUser().getId()).getResultList();
    }
    ...
    private boolean isVolumeAttach(final Job job) {
        ...
        if ((job.getTargetEntity() instanceof Machine)
            ...
            try {
                this.volumeManager.getVolumeById(volume.getId()
                        .toString());
     ...
     * 
     */
    private Machine getMachineFromConnector(final CloudProviderAccount cpa,
            final CloudProviderLocation loc,
     ...
     * database
     */
    private boolean completeMachineCreation(final Job notification,
            final Machine local, final Machine remote) {
     ...
     */
    private boolean completeDeviceAttachmentToMachine(final boolean creation,
            final Job notification, final Machine local, final Machine remote) {
    ...
    public boolean jobCompletionHandler(final String notification_id)
        ...
        if (!notification.getAction().equals("delete")) {
            updated = this.getMachineFromConnector(
        ...
        if (notification.getProperties().containsKey("parent-machine")) {
                ...
                if (children.size() != 0) {
                    ...
                    MachineManager.logger
                            .info(" notification for a non-leaf job "
                                    + notification.getId());
                    return this.machineCreationContinuation(notification,
                            mpersisted);
                } else {
                    /**
                    ...
                    MachineManager.logger
                            .info(" notification for an attachment completion "
                                    + notification.getId());
                    return this.completeDeviceAttachmentToMachine(false,
             ...
             * machine creation leaf job: no affectedEntity -- machine volume
             * attachment leaf job as part of machine create
             */
            ...
            if (affected == null || affected.size() == 0) {
                ...
                MachineManager.logger
                        .info("completeMachineCreation notification for machine "
                                + notification.getId());
                return this.completeMachineCreation(notification, mpersisted,
                        updated);
            } else {
                /** machine volume attachment leaf job as part of machine create */
                return this.completeDeviceAttachmentToMachine(true,
                        notification, mpersisted, updated);
            }
        }
    ...
    private void validateNetworkInterface(final List<MachineTemplateNetworkInterface> nics)
            throws CloudProviderException {
    ...
    private Job addVolumeToMachine(final Machine m, final MachineVolume mv)
        ...
        if (j.getStatus() != Job.Status.FAILED) {
            try {
                UtilsForManagers.emitJobListenerMessage(
                        j.getProviderAssignedId(), this.ctx);
            } catch (Exception e) {
                throw new ServiceUnavailableException(e.getMessage());
            }
        }
    ...
    @Override
    public Job updateVolumeOnMachine(final String machineId,
    ...
    @Override
    public MachineVolume getVolumeFromMachine(final String machineId,
            final String mVolId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
    ...
    @Override
    public Job addDiskToMachine(final String machineId, final MachineDisk disk)
            throws ResourceNotFoundException, CloudProviderException,
            InvalidRequestException {
        throw new ServiceUnavailableException(" Operation not permitted ");
    }
    ...
    @Override
    public List<MachineDisk> getMachineDisks(final String machineId)
            throws ResourceNotFoundException, CloudProviderException,
            InvalidRequestException {
        Machine m = this.getMachineById(machineId);
    ...
    @Override
    public Job removeDiskFromMachine(final String machineId,
            final String machineDiskId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
        throw new ServiceUnavailableException(" Operation not permitted ");
    }

    @Override
    public MachineDisk getDiskFromMachine(final String machineId,
            final String machineDiskId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
    ...
    @Override
    public Job updateDiskOnMachine(final String machineId,
            final String machineDiskId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
        throw new ServiceUnavailableException(" Operation not permitted ");
    }
    ...
    @Override
    public Job addNetworkInterfaceToMachine(final String machineId,
    ...
    @Override
    public Job removeNetworkInterfaceFromMachine(final String machineId,
            final String nicId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
        // TODO Auto-generated method stub
        throw new ServiceUnavailableException(" Operation not permitted ");
    }
    ...
    @Override
    public Job updateNetworkInterfaceOnMachine(final String machineId,
            final String nicId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
        // TODO Auto-generated method stub
        throw new ServiceUnavailableException(" Operation not permitted ");
    ...
    @Override
    public MachineNetworkInterface getNetworkInterfaceFromMachine(final String machineId,
            final String nicId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
        // TODO Auto-generated method stub
    ...
    @Override
    public List<MachineNetworkInterface> getMachineNetworkInterfaces(
            final String machineId) throws ResourceNotFoundException,
            CloudProviderException, InvalidRequestException {
        // TODO Auto-generated method stub
        return null;
    }
