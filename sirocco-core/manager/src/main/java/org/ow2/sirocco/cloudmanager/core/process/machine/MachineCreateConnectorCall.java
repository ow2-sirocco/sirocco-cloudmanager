...
@Stateless
@Local
@EJB(name = "java:global/delegate/MachineCreateConnectorCall", mappedName = "MachineCreateConnectorCall", beanInterface = JavaDelegate.class)
public class MachineCreateConnectorCall implements JavaDelegate {

    private static Logger logg = LoggerFactory.getLogger(MachineCreateConnectorCall.class.getCanonicalName());
    ...
    public void execute(final DelegateExecution execution) throws Exception {
        try {
            MachineCreateConnectorCall.logg.info(OrchestratorUtils.infoLog("entering ConnectorCall", execution));
            ...
            String providerAssignedId = computeService.createMachine((MachineCreate) input_objectCreate, target)
                .getProviderAssignedId();

            // saving cloud ID to our own object
            object.setProviderAssignedId(providerAssignedId);
