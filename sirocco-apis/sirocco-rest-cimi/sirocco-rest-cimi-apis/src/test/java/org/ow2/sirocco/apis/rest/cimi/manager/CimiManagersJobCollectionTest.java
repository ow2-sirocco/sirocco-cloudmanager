...
public class CimiManagersJobCollectionTest {
    ...
    public void testRead() throws Exception {
        ...
        for (int i = 0; i < cimiCollect.getArray().length; i++) {
            Assert.assertEquals(ConstantsPath.JOB_PATH + "/" + (i + 13), cimiCollect.getArray()[i].getHref());
        }
    ...
    public void testReadWithCimiSelectAttributes() throws Exception {
        ...
        List<Job> list = new ArrayList<Job>();
        EasyMock.expect(
            this.service.getJobs(EasyMock.eq(Arrays.asList(new String[] {"operations"})), EasyMock.eq((String) null)))
            .andReturn(list);
        EasyMock.replay(this.service);
    ...
    public void testReadWithCimiSelectArrays() throws Exception {
        ...
        EasyMock.expect(
            this.service.getJobs(EasyMock.eq(1), EasyMock.eq(23), EasyMock.eq(Arrays.asList(new String[] {"machineImages"}))))
            .andReturn(list);
        EasyMock.replay(this.service);
