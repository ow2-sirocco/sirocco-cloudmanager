...
public class Base64Encoder {
    ...
    static {
        for (int i = 0; i < Base64Encoder.map2.length; i++) {
            Base64Encoder.map2[i] = -1;
        }
        for (int i = 0; i < 64; i++) {
            Base64Encoder.map2[Base64Encoder.map1[i]] = (byte) i;
        }
    }
    ...
    public static String encodeLines(final byte[] in) {
        return Base64Encoder.encodeLines(in, 0, in.length, 76, Base64Encoder.systemLineSeparator);
    }
    ...
    public static String encodeLines(final byte[] in, final int iOff, final int iLen, final int lineLen,
        ...
        while (ip < iLen) {
            ...
            buf.append(Base64Encoder.encode(in, iOff + ip, l));
    ...
    public static char[] encode(final byte[] in, final int iOff, final int iLen) {
        ...
        while (ip < iEnd) {
            ...
            int o3 = i2 & 0x3F;
            out[op++] = Base64Encoder.map1[o0];
            out[op++] = Base64Encoder.map1[o1];
            out[op] = op < oDataLen ? Base64Encoder.map1[o2] : '=';
            op++;
            out[op] = op < oDataLen ? Base64Encoder.map1[o3] : '=';
            op++;
        }
        return out;
    }
    ...
    public static byte[] decodeLines(final String s) {
        ...
        return Base64Encoder.decode(buf, 0, p);
    }
    ...
    public static byte[] decode(final String s) {
        return Base64Encoder.decode(s.toCharArray());
    }
    ...
    public static byte[] decode(final char[] in) {
        return Base64Encoder.decode(in, 0, in.length);
    }
    ...
    public static byte[] decode(final char[] in, final int iOff, int iLen) {
        ...
        while (ip < iEnd) {
            ...
            int b0 = Base64Encoder.map2[i0];
            int b1 = Base64Encoder.map2[i1];
            int b2 = Base64Encoder.map2[i2];
            int b3 = Base64Encoder.map2[i3];
